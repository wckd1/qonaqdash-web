...
**New page / route (UI starting point)** — Follow **[docs/design.md](docs/design.md) → *Workflow: new page or route***: closest template + catalog row → describe screen → copy reference `*.vue` and prune → use **[docs/design.html](docs/design.html)** for shared control markup → scoped CSS for page-only pieces; if a scoped pattern repeats elsewhere, propose promoting it into the design system per **`design.md` → *Documentation split* → *Documentation layers*** → update `design.md` when layout matches, again when the page is final. Still satisfy the checklist above (contract, i18n, build, etc.).
...
- **Design system**: [docs/design.html](docs/design.html) + `main.css` define top-level visuals; [docs/design.md](docs/design.md) specifies pages using that naming; views add scoped extensions and propose promotion when patterns repeat (see *Documentation split*).
