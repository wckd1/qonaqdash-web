...
| `/manage/rooms`              | `RoomsView.vue`                | **T1** + custom | Same page title "Hotel" + **PropertySubNav**. Rooms tab: accordion room types with type-level edit/remove/add-room dialogs; room tables with three-axis status badges in a single row. Row opens **`RoomSidePanel`** (summary + "Open full page" → `/manage/rooms/:id/details`); no inline actions.                                                                                                                                                                  |
| `/manage/rooms/:id/details`  | `RoomDetailView.vue`           | **T3**          | Full room detail. Header: "Room {number} — {type}". `action-toolbar action-toolbar--inset` with **Cleaning** / **Maintenance** / **Mark out of order / available** / **Edit** / **Delete**. Cleaning opens dialog with housekeeping status picker; Maintenance opens dialog with status + `planned_end` + note; Edit opens room number + type dialog; Delete routes back to `/manage/rooms`. Summary panel (type + axis badges + note) + Activity panel (event log). |
...
| Route           | View                        | Template | Routine actions & content                                                                                                                                                                                                               |
| --------------- | --------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/housekeeping` | `HousekeepingTasksView.vue` | **T2**   | Header: title + **New task**. Status/unassigned filter toolbar. Table: room, description, status chip (`open` / `in_progress` / `inspection` / `done` / `cancelled`), assignee, updated. Row → side panel with source (manual / auto on checkout), description, assignee select, status-transition buttons. Wraps `TaskBoardView` (domain=`housekeeping`). Create dialog collects room + description + assignee. |
| `/maintenance`  | `MaintenanceTasksView.vue`  | **T2**   | Same pattern (domain=`maintenance`). Create dialog additionally requires `planned_end` (datetime-local) and optional `note`; both propagate onto the room maintenance axis when the task is started server-side. Status lifecycle: `open → in_progress → done` plus cancel from any non-terminal state.                                                                                                         |
