    ...
    <div class="list-content__viewport">
      ...
      <template v-else>
        <div v-if="!bookings.length && !searchQuery" class="empty-state-widget">
          ...
          <h3 class="empty-state-widget__title">{{ t('bookings.emptyTitle') }}</h3>
          <p class="empty-state-widget__description">{{ t('bookings.emptyDescription') }}</p>
          <div class="empty-state-widget__actions">
            <router-link :to="{ name: 'booking-new' }" class="primary" role="button">{{ t('bookings.newBooking') }}</router-link>
          </div>
        </div>
